...
async function main() {
  const js = `
    (async()=>{
      ...
      const rel=cols.filter(c=>/follow|relation|friend/i.test(c));
      console.log('RELATED='+JSON.stringify(rel));
      const sample={};
      for(const c of rel){
        sample[c]=await db.collection(c).findOne();
      }
      console.log('SAMPLES='+JSON.stringify(sample).slice(0,1200));
      const u=await db.collection('users').findOne({id:'1065527'},{projection:{fans:1,following:1,followersList:1,followingList:1}});
      console.log('USER_1065527='+JSON.stringify(u));
