...
async function main() {
  ...
  try {
    const r1 = await sshExec(conn, "docker exec app-backend sh -c \"grep -c 'isFollowed' /app/dist/routes/searchRoutes.js\"");
    console.log('isFollowed no compilado: ' + r1.out.trim());
  } catch (e) {
