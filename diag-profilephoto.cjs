...
async function main() {
  ...
  try {
    const r0 = await sshExec(conn, 'cat /app/backend/src/routes/profilePhotoRoutes.ts', 30000);
    const lines = r0.out.split('\n');
    console.log('=== profilePhoto 90-195 ===\n' + lines.slice(89, 195).join('\n'));
    const r1 = await sshExec(conn, 'head -20 /app/backend/src/models/ProfilePhoto.ts 2>/dev/null; echo ====; grep -n "ProfilePhoto" /app/backend/src/routes/userRoutes.ts | head', 30000);
    console.log('=== ProfilePhoto model + import in userRoutes ===\n' + r1.out);
  } catch (e) {
