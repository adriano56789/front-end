...
async function main() {
  ...
  try {
    const r1 = await sshExec(conn, 'ls -la /app/backend/src/routes/ | grep -i -E "search|user"');
    console.log('=== routes search/user ===\n' + r1.out);
    const r2 = await sshExec(conn, 'grep -rn "export default" /app/backend/src/routes/ | grep -i search');
    console.log('=== search export ===\n' + r2.out);
    const r3 = await sshExec(conn, 'grep -rn "search/users" /app/backend/src/ --include="*.ts" 2>/dev/null | head');
    console.log('=== montagem app.use ===\n' + r3.out);
  } catch (e) {
