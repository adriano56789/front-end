...
async function main() {
  ...
  try {
    let r = await sshExec(conn, 'docker exec app-backend sh -c "cat /app/backend/src/utils/userResponse.ts | base64 -w0"');
    if (r.code !== 0 || !r.out.trim()) {
      r = await sshExec(conn, 'find /app/backend/src -name userResponse.ts -exec base64 -w0 {} ;');
    }
    const srcB64 = r.out.trim();
    const src = Buffer.from(srcB64, 'base64').toString('utf8');
    fs.writeFileSync(path.join(tmp, 'userResponse.remote.ts'), src);
    console.log('SRC decoded chars:', src.length);
    console.log('DONE');
  } catch (e) {
