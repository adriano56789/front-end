...
async function main() {
  ...
  try {
    const cmd = `cd /app/backend/src && grep -rn -iE "on_unpublish|unpublish|removeStream|removeLive|endStream|autoEnd|auto_end|is_live|cleanup.*stream|leaveStream" routes services utils models controllers 2>/dev/null | grep -viE "\.map:" | head -120`;
    const r = await sshExec(conn, cmd, 60000);
    console.log('=== LIVE AUTO-REMOVE REFS ===\n' + (r.out || '(sem resultados)'));
  } catch (e) {
