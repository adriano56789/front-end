...
async function main() {
  ...
  try {
    const r1 = await sshExec(conn, "sed -i 's/livenza-cache-v16/livenza-cache-v17/' /var/www/livego.store/sw.js && grep -o 'livenza-cache-v[0-9]*' /var/www/livego.store/sw.js | head -1 && echo SW_BUMPED");
