...
async function main() {
  ...
  try {
    const r = await sshExec(conn, 'cd /app/backend/src/routes && grep -n "obras" userRoutes.ts | head -40', 30000);
    console.log('=== obras refs in userRoutes ===\n' + r.out);
    const r2 = await sshExec(conn, 'cd /app/backend/src/routes && grep -n "standardizeUserResponse\\|getUser:\\|router.get(\'/:id\'\|router.get(\"/:id\"\|photos/gallery" userRoutes.ts | head -40', 30000);
    console.log('=== getUser refs ===\n' + r2.out);
  } catch (e) {
