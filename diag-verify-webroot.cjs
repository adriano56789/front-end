...
async function main() {
  ...
  try {
    const r = await sshExec(conn, [
      "echo 'VERSION:'; cat /var/www/livego.store/version.json 2>/dev/null",
      "echo 'SW:'; grep -o \"livenza-cache-v[0-9]*\" /var/www/livego.store/sw.js 2>/dev/null | head -1",
      "echo 'ASSET:'; ls /var/www/livego.store/assets/index-BJiU9353.js 2>/dev/null && echo ASSET_OK || echo ASSET_MISSING",
      "echo 'HOSTID_MARK:' ; grep -c 'hostId:x50' /var/www/livego.store/assets/index-BJiU9353.js 2>/dev/null || echo 0",
      "echo 'HTTP_INDEX:'; curl -s -o /dev/null -w '%{http_code} (%{size_download} bytes)' https://livego.store/ 2>/dev/null",
      "echo; echo 'HTTP_VERSION:'; curl -s https://livego.store/version.json 2>/dev/null",
    ].join('; '), 90000);
    console.log(r.out + '\n(exit ' + r.code + ')');
  } catch (e) {
