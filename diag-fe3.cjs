...
async function main() {
  ...
  try {
    const r1 = await sshExec(conn, "grep -o 'index-[A-Za-z0-9_-]*\\.js' /var/www/livego.store/index.html | head -2");
    console.log('=== index.html ref ===\n' + r1.out);
    const r2 = await sshExec(conn, "grep -o \"CACHE_NAME[^;]*\" /var/www/livego.store/sw.js | head -2; echo '---local-check---'; grep -o 'livenza-cache-v[0-9]*' /var/www/livego.store/sw.js | head -1");
    console.log('=== sw cache name (remote) ===\n' + r2.out);
  } catch (e) {
