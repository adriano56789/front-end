...
async function main() {
  ...
  try {
    const m1 = await sshExec(conn, 'cat /app/backend/src/models/Photo.ts; echo ====; cat /app/backend/src/models/UserPhoto.ts 2>/dev/null | head -40', 30000);
    console.log('=== MODELS ===\n' + m1.out);
    const r2 = await sshExec(conn, 'cd /app/backend/src/routes && grep -n -iE "photos|userPhotos|obras" profilePhotoRoutes.ts photoRoutes.ts userRoutes.ts 2>/dev/null | grep -iE "router\.(get|post|put|delete)|const|app\.(get|post)" | head -30', 30000);
    console.log('=== PHOTO ROUTES ===\n' + r2.out);
  } catch (e) {
