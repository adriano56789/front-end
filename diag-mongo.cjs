...
async function main() {
  const js = `
    (async()=>{
      ...
      await mongoose.connect('mongodb://livego_admin:ce0d61d0277da7031e11c97b66a775f01ab0f0bfbe443108@mongodb:27017/api?authSource=admin', {serverSelectionTimeoutMS:15000});
      const db=mongoose.connection.db;
      const ids=['1065527','1951388','6771613'];
      for(const id of ids){
        const total=await db.collection('followers').countDocuments({followingId:id, isActive:true});
        const docs=await db.collection('followers').find({followingId:id, isActive:true}).limit(30).toArray();
        const fids=docs.map(d=>d.followerId);
        const existing=await db.collection('users').find({id:{$in:fids}}).toArray();
        const itemap=new Set(existing.map(u=>u.id));
        const stale=fids.filter(x=>!itemap.has(x));
        const inactiveCount=await db.collection('followers').countDocuments({followingId:id, isActive:false});
        console.log('uid='+id, 'activeFollowersDocs='+total, 'listedDocs='+fids.length, 'staleFollowers='+stale.length, 'inactiveDocs='+inactiveCount, 'staleIds='+JSON.stringify(stale));
      }
