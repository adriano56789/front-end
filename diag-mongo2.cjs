...
async function main() {
  const js = `
    (async()=>{
      ...
      console.log('COLLECTIONS='+JSON.stringify(cols));
      const rel=cols.filter(c=/follower|follow|relation|friend/i);
      console.log('RELATED='+JSON.stringify(rel));
      for(const c of rel.slice(0,3)){
        const total=await db.collection(c).countDocuments();
        const one=await db.collection(c).findOne();
        console.log('---', c, 'total=', total, 'one=', JSON.stringify(one).slice(0,600));
      }
      const u=await db.collection('users').findOne({id:'1065527'});
      console.log('USER_1065527='+JSON.stringify({id:u&&u.id, fans:u&&u.fans, following:u&&u.following, followersList:u&&u.followersList, followingList:u&&u.followingList}).slice(0,1000));
  ...
  try {
    ...
    const r = await sshExec(conn, cmd, 120000);
    console.log('=== mongo ===\n' + r.out);
  } catch (e) {
