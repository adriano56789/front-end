...
export class VideoProcessor {
  ...
  /** A fonte de vídeo está de fato tocando com resolução válida? */
  isSourceActive(): boolean {
    const v = this.processingVideoElement || this.videoElement;
    if (!v) return false;
    return !v.paused && v.readyState >= 2 && (v.videoWidth > 0 || v.videoHeight > 0);
  }

  /** O canvas já desenhou pelo menos 1 frame REAL com pixels da câmera? */
  ...
  constructor(config?: Partial<VideoProcessorConfig>) {
    ...
    // Inscrever no beautyState: quando o BeautyEffectsPanel chama
    // beautyState.update(), o render loop já lê this.beautySettings.
  ...
  private compileShaders(): boolean {
    ...
    const fragmentShaderSource = `
      ...
      // --- Limpar Chiado (smartDeNoise — BrutPitt/glslSmartDeNoise) ---
      // Filtro bilateral gaussiano circular: remove grão/chiado da câmera
      // preservando bordas (contornos, olhos, boca). Roda ANTES dos demais
      // efeitos para que nitidez/saturação não amplifiquem o ruído.
      // sigma = desvio padrão espacial (raio do kernel)
      // kSigma = multiplicador do raio (kSigma*sigma = raio final)
      // threshold = limiar de borda (quanto maior, mais preserva contornos)
      #define DENOISE_INV_SQRT_2PI 0.3989422804
      #define DENOISE_INV_PI 0.3183098862
      vec3 smartDeNoise(vec2 uv, float sigma, float kSigma, float threshold) {
        float radius = round(kSigma * sigma);
        float radQ = radius * radius;
        float invSigmaQx2 = 0.5 / (sigma * sigma);
        float invSigmaQx2PI = DENOISE_INV_PI * invSigmaQx2;
        float invThresholdSqx2 = 0.5 / (threshold * threshold);
        float invThresholdSqrt2PI = DENOISE_INV_SQRT_2PI / threshold;
        vec4 centrPx = texture2D(u_texture, uv);
        float zBuff = 0.0;
        vec4 aBuff = vec4(0.0);
        ...
        for (float dx = -radius; dx <= radius; dx++) {
          ...
          for (float dy = -pt; dy <= pt; dy++) {
            ...
            vec2 off = vec2(dx, dy) / size;
            vec4 walkPx = texture2D(u_texture, uv + off);
        ...
        return (zBuff > 0.001) ? aBuff.xyz / zBuff : centrPx.xyz;
      }
      vec3 applyDenoise(vec3 rgb, vec2 uv, vec2 texelSize, float strength) {
        // strength 0-100 → sigma 0.5-4.0, threshold 0.05-0.15
        // sigma baixo = raio pequeno (rápido), threshold alto = preserva bordas
        float sigma = 0.5 + strength * 0.035;
        float kSigma = 2.0;
        float threshold = 0.10 + strength * 0.0005;
        return smartDeNoise(uv, sigma, kSigma, threshold);
      }
