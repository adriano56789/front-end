...
// Token mantido em memória - sem localStorage
let currentToken: string | null = null;
let currentUserData: JWTPayload | null = null;

/**
 * Obtém o token JWT da memória
 */
...
/**
 * Remove o token da memória (logout)
 */
...
/**
 * Salva o token na memória com validação
 */
...
/**
 * Obtém informações completas do usuário do token em memória
 */
export const getUserInfo = (): { id: string; name?: string } | null => {
  if (!currentUserData) {
    const token = getToken();
    if (!token) return null;
    currentUserData = decodeToken(token);
  }
  
  if (!currentUserData) return null;

  const id = currentUserData.id || currentUserData.userId || currentUserData.sub;
  if (!id) return null;

  const name = currentUserData.name || currentUserData.username || currentUserData.displayName;
