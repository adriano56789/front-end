...
const LiveCallInvitation: React.FC<LiveCallInvitationProps> = ({ 
  ...
  const handleCallInvitation = (event: any) => {
    ...
    switch (event.type) {
      ...
      case 'invitation_accepted':
        ...
        showToast('O usuário aceitou o convite para entrar na live', 'success');
        break;
  ...
  const handleConnected = (data: any) => {
    ...
    showToast('Conectado à chamada', 'success');
  };
  ...
  const handleDisconnected = () => {
    ...
    setIsInCall(false);
    onGuestLeft?.();
    showToast('Desconectado da chamada', 'info');
  };

  const handleError = (error: any) => {
    console.error('Erro na chamada:', error);
    showToast(`Erro na chamada: ${error.error}`, 'error');
  };
  ...
  const handleSendInvitation = async () => {
    if (!guestUserId.trim()) {
      showToast('Digite o ID do usuário para convidar', 'error');
      return;
    }
    ...
    if (result.success) {
      ...
      showToast(`Erro ao enviar convite: ${result.error}`, 'error');
    }
  };
  ...
  const handleAcceptInvitation = async () => {
    ...
    if (result.success) {
      ...
      showToast('Conectando à chamada...', 'success');
    } else {
      showToast(`Erro ao aceitar convite: ${result.error}`, 'error');
    }
  };
  ...
  const handleDeclineInvitation = async () => {
    ...
    if (result.success) {
      ...
      showToast(`Erro ao recusar convite: ${result.error}`, 'error');
    }
  };
  ...
  const handleEndCall = async () => {
    ...
    if (result.success) {
      ...
      showToast(`Erro ao encerrar chamada: ${result.error}`, 'error');
    }
  };
