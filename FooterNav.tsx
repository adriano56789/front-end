...
import { HomeIcon, VideoIcon, MessageIcon, PlayIcon } from './components/icons';
...
interface FooterNavProps {
  ...
  onOpenChat: () => void; // Nova prop para abrir chat privado
}

const FooterNav: React.FC<FooterNavProps> = ({ currentUser, onOpenGoLive, activeTab, onNavigate, onOpenChat }) => {
  const { t } = useTranslation();

  return (
    <footer className="footer-safe-area flex-shrink-0">
      <div className="flex items-center justify-around h-16 text-gray-400">
        ...
        <button 
          onClick={() => {
            alert('Chat clicado!');
            console.log('🔥 FooterNav chat clicado!');
            onOpenChat();
          }}
          className="flex flex-col items-center w-1/5 pt-1 relative bg-red-500 text-white"
        >
