...
async function main() {
  ...
  try {
    const r1 = await sshExec(conn, 'grep -o "assets/index-[^\"\x27]*" /var/www/livego.store/index.html | head -3');
    console.log('=== index ref ===\n' + r1.out);
    const r2 = await sshExec(conn, 'ls -la /var/www/livego.store/assets/index-zGcdrRo6.js; printf "HOSTID_COUNT: "; grep -o "hostId" /var/www/livego.store/assets/index-zGcdrRo6.js | wc -l');
    console.log('=== new asset ===\n' + r2.out);
    const r3 = await sshExec(conn, 'grep -o "getRankingForPeriod" /var/www/livego.store/assets/index-zGcdrRo6.js | wc -l; echo "---sw---"; grep -o "livenza-cache-v[0-9]*" /var/www/livego.store/sw.js | head -1; echo "---prev---"; ls -ld /tmp/livego-webroot-prev 2>&1');
    console.log('=== verify ===\n' + r3.out);
  } catch (e) {
