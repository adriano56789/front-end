...
async function main() {
  ...
  try {
    const r1 = await sshExec(conn, 'ls -la /app/backend/ 2>&1 | head -30');
    console.log('=== /app/backend (host) ===\n' + r1.out);
    const r2 = await sshExec(conn, 'ls -la /app/backend/src/routes/userRoutes.ts 2>&1; ls -la /app/backend/routes/userRoutes.ts 2>&1');
    console.log('=== userRoutes exists? ===\n' + r2.out);
    const r3 = await sshExec(conn, 'cat /app/docker-compose.yml | head -50; echo "====DOCKERFILE===="; cat /app/backend/Dockerfile 2>&1 | head -50');
    console.log('=== compose + dockerfile ===\n' + r3.out);
  } catch (e) {
