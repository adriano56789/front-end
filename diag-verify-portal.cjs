...
async function main() {
  ...
  try {
    const r = await sshExec(conn, [
      ...
      "echo 'NEW_ASSET:'; ls /var/www/livego.store/assets/index-_l1UI19E.js 2>/dev/null && echo OK || echo MISSING",
      "echo 'PORTAL_MARK:'; grep -c 'createPortal' /var/www/livego.store/assets/index-_l1UI19E.js 2>/dev/null || echo 0",
      "echo 'HTTP:'; curl -s -o /dev/null -w '%{http_code}' https://livego.store/",
    ].join('; '), 90000);
