...
const MediaItem: React.FC<{ 
    ...
    // Robust video check
    const mediaUrl = photo.mediaUrl || photo.photoUrl || '';
    const isVideo = mediaUrl.toLowerCase().includes('data:video') || 
                ...
                <div className="flex justify-between items-end pointer-events-auto">
                    {photo.user && (
                        <button onClick={() => onViewProfile(photo.user)} className="flex items-center space-x-3 mb-2">
                            <img src={photo.user.avatarUrl || ''} alt={photo.user.name || 'User'} className="w-10 h-10 rounded-full object-cover border border-white/50" />
                            <span className="font-bold text-white text-lg shadow-black drop-shadow-md">{photo.user.name || 'Unknown User'}</span>
                        </button>
                    )}
...
const FullScreenPhotoViewer: React.FC<FullScreenPhotoViewerProps> = ({ photos, initialIndex, onClose, onViewProfile, onPhotoLiked }) => {
  ...
  const handleLike = async (photoId: string) => {
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await api.likePhoto(photoId, undefined, photo.mediaUrl || photo.photoUrl);
        if (response.success) {
