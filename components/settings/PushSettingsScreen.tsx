...
const CustomSwitch: React.FC<{ checked: boolean; onChange: () => void }> = ({ checked, onChange }) => (
  ...
  >
    <span
      aria-hidden="true"
      className={`pointer-events-none inline-block h-[18px] w-[18px] transform rounded-full bg-white shadow-[0_2px_4px_rgba(0,0,0,0.35)] transition duration-300 ease-in-out mt-[4px] ${checked ? 'translate-x-[24px]' : 'translate-x-[4px]'}`}
    />
  </button>
);
...
const PushSettingsScreen: React.FC<PushSettingsScreenProps> = ({ onBack, currentUser }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Buscar following e push settings em paralelo (não serial!)
        const [usersRes, settingsRes] = await Promise.allSettled([
          api.getFollowingUsers(currentUser.id),
          api.getPushSettings(currentUser.id),
        ]);
        ...
        if (settingsRes.status === 'fulfilled') {
          saved = settingsRes.value?.settings || {};
        }
  ...
  const handleToggle = (userId: string) => {
    ...
    api.updatePushSettings(currentUser.id, updated).catch(() => {});
  };
      ...
      <main className="flex-grow overflow-y-auto no-scrollbar px-4 py-2 z-10">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-center px-6">
            ...
            <p className="text-[13px] text-zinc-500 mt-2">Siga streamers para configurar notificações push individuais.</p>
          </div>
        ) : (
          <div>
                  ...
                  <div className="flex items-center space-x-3.5">
                    <div className="relative">
                      <img 
                        ...
                        onError={(e) => { (e.currentTarget as HTMLImageElement).src = `https://picsum.photos/seed/${user.id}/120/120`; }}
                      />
                      {user.isLive && (
                        <LiveBadge label="" showLabel={false} iconClassName="w-3 h-3" className="absolute bottom-0 right-0 rounded-full p-[1.5px]" />
                      )}
                    </div>
                    <span className="text-[15px] font-medium text-zinc-100">{user.name}</span>
                  </div>
                  <CustomSwitch 
