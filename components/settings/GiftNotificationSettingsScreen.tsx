...
export const GiftNotificationSettingsScreen: React.FC<{ onBack: () => void; user: User; gifts: Gift[] }> = ({ onBack, user, gifts }) => {
    ...
    useEffect(() => {
      if (user) {
          setIsLoading(true);
          api.getGiftNotificationSettings(user.id)
              .then(data => {
                  const initialSettings = gifts.reduce((acc, gift) => ({ ...acc, [gift.name]: true }), {});
                  if (data.settings && Object.keys(data.settings).length > 0) {
                      setEnabledGifts({ ...initialSettings, ...data.settings });
                  } else {
    ...
    const handleToggle = (giftName: string) => {
        const newSettings = { ...enabledGifts, [giftName]: !enabledGifts[giftName] };
        setEnabledGifts(newSettings); // Optimistic update
        // Save to backend
        api.updateGiftNotificationSettings(user.id, newSettings).catch(() => {
            // Revert on failure
            setEnabledGifts(enabledGifts);
        });
    };
            ...
            <main className="flex-grow overflow-y-auto no-scrollbar p-4 space-y-4">
                ...
                {!user.isVIP && (
                    <div className="bg-yellow-800/50 border border-yellow-700 text-yellow-300 text-sm p-3 rounded-lg flex justify-between items-center">
                        ...
                        {/* The onOpenVIPCenter prop is not available here, so the button is not interactive but visually correct. */}
                        <button className="bg-yellow-500 text-black font-bold px-4 py-1 rounded-full text-xs hover:bg-yellow-600 transition-colors">{t('vip.subscribe')}</button>
                    </div>
                )}
