...
const EntryChatMessage: React.FC<EntryChatMessageProps> = ({ user, currentUser, onClick, onFollow, isFollowed, isBroadcaster, isModerator }) => {
    ...
    return (
        ...
        >
            <div className="relative w-5 h-5 shrink-0 align-middle inline-block mr-0.5">
                {user.activeFrameId && frameColors[user.activeFrameId] && (
                    <div
                        className="absolute -inset-[1.5px] rounded-full"
                        style={{
                            background: `linear-gradient(135deg, ${frameColors[user.activeFrameId]}, transparent 60%, ${frameColors[user.activeFrameId]})`,
                            boxShadow: `0 0 5px ${frameColors[user.activeFrameId]}`,
                        }}
                    />
                )}
                <div className="w-5 h-5 rounded-full overflow-hidden bg-black/30 relative z-[1]">
                    <img
