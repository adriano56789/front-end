...
const StreamerCard: React.FC<{streamer: Streamer; onSelect: (streamer: Streamer) => void}> = ({ streamer, onSelect }) => {
    ...
    const handleEnterLive = () => {
        console.log(`🎯 [MainScreen] Navegando para live: /live/${streamer.id}`);
        navigate(`/live/${streamer.id}`); // Navegação igual ao BuzzCast
    };

    return (
        <div className="relative aspect-[3/4] rounded-lg overflow-hidden cursor-pointer group" onClick={handleEnterLive}>
