...
const NEW_SRS_CONF = `listen              1935;
...
conn.on('ready', () => {
  ...
  const remotePath = '/tmp/srs.conf.new';
  const localPath = path.resolve(__dirname, 'srs.conf.new');
  fs.writeFileSync(localPath, NEW_SRS_CONF);
  ...
  conn.sftp((err, sftp) => {
    ...
    sftp.fastPut(localPath, remotePath, (err) => {
      ...
      const cmd = [
        'echo "=== BACKUP ==="',
        'docker exec app-srs cp /usr/local/srs/conf/srs.conf /usr/local/srs/conf/srs.conf.bak',
        'echo "=== COPY NEW CONFIG ==="',
        `docker cp ${remotePath} app-srs:/usr/local/srs/conf/srs.conf`,
        'echo "=== VERIFY ==="',
        'docker exec app-srs cat /usr/local/srs/conf/srs.conf | grep -E "hls_dispose|hls_ctx|hls_ts_ctx|hls_fragment|hls_window"',
        'echo "=== RESTART SRS ==="',
