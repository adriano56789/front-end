...
conn.on('ready', () => {
  ...
  // First backup
  conn.exec('cp /app/srs-docker.conf /app/srs-docker.conf.bak && echo BACKUP_OK', (err, stream) => {
...
vhost __defaultVhost__ {
  ...
  const cmd = `echo "${b64}" | base64 -d > /app/srs-docker.conf && echo CONFIG_WRITTEN && grep hls_dispose /app/srs-docker.conf && docker restart app-srs && sleep 3 && docker ps --filter name=app-srs --format "{{.Status}}" && echo DONE`;
