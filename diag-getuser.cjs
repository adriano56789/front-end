...
async function main() {
  ...
  try {
    const r0 = await sshExec(conn, 'cat /app/backend/src/routes/userRoutes.ts', 30000);
    const lines = r0.out.split('\n');
    console.log('=== userRoutes 200-260 ===\n' + lines.slice(199, 260).join('\n'));
    console.log('\n=== userRoutes 60-205 (middle) ===\n' + lines.slice(59, 205).join('\n'));
  } catch (e) {
